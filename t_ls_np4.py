...
def f_approx(x, a0, a, b, p, v=0):  #x vector no scalar
    if(v==0):
        y = np.ones(len(x))*a0
        for i in range(len(x)):
            y[i] += np.sum(a/(x[i]+np.exp(b)))
    y = np.zeros(len(x))        
    if(v==1):
        for i in range(len(x)):
            y[i] += np.sum(   a/p            *(-x[i])     /(x[i]+np.exp(b))**2 )
    if(v==2):
        for i in range(len(x)):
            y[i] += np.sum( 2*a/p/(p-1)      *( x[i]**2)  /(x[i]+np.exp(b))**3 )
    if(v==3): 
        for i in range(len(x)):
            y[i] += np.sum(-6*a/p/(p-1)/(p-2)*(-x[i]**3)  /(x[i]+np.exp(b))**4 )
    return y
    
def err_func(b, x, y, p,v):
    a0,a = fit_a_d(x,y,b)
    return (f_approx(x, a0, a, b, p, v=3)-y)
    
def fit_a(X,Y,coef): #given the b coefficient, the a0, and d can be fited exactly using lin_alg
    k = len(coef)
    N = len(X)
    A = np.zeros((k+1,k+1))
    B = np.zeros( k+1 )
...
def run_min(x_min,x_max,p,N,coef):
    ...
    coef_s,success=leastsq(err_func,coef,args=(X,Y,p), ftol=1.49012e-15, xtol=1.49012e-15, gtol=0.0)
    coef_s.sort()
    coef = coef_s
